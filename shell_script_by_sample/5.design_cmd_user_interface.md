# 设计 Command line Interface 的三的 Pattern

在我们讨论 User Interface 的时候，通常讨论的是 Graph User Interface (GUI)。在我们创建 Command line 工具的时候，好的 User Interface 可以让工具具有很好的用户体验。这种交互方式成为 Command line Interface。比如 `Git` 就是一个非常优秀的 Command line Interface 案例。
...
我们对上面的 `run-mysql` 再进行改造，让他直接 `run` 和 `kill` 两个 `Sub-Command`。
...
引用一段[来自程序员的高效注释](http://blog.jobbole.com/105813/)以供自勉：
